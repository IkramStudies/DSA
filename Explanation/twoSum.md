...
We mark the place where we found the previous number and the current number, previous doesn't mean the last element. We are using a simple formula, current element + previous element = target. We mark the current elements index in every iteration.
